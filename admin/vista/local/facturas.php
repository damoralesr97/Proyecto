    ...
    <header>
        ...
        <table class="tablaLocales">
            ...
            <?php
                ...
                $sql = "SELECT * FROM factura_cabecera WHERE fc_loc_codigo=$codigoUsr";
                $result = $conn->query($sql);
                ...
                if ($result->num_rows > 0){
                    while($row = $result->fetch_assoc()){
                        if($row["fc_eliminado"]!='S'){
                            ...
                            echo "<td><a href='anular_factura.php?usu=".$row['fc_usu_codigo']."&fac=".$row["fc_codigo"]."'>Anular</a></td>";
                        }
                    }
                }else{
                    echo "<tr>";
                    echo "<td colspan='5'>No tienes pedidos en tu local!!!</td>";
                    echo "</tr>";
                }
                $conn->close();
            ?>
